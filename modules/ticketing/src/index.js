        ...
        for (const ticket of activeTickets) {
            ...
            if (pct >= reminderPct && !ticket.reminder_sent_at && ticket.assignee_id) {
                ...
                await db.withSuperAdmin(async (c) => {
                    ...
                    await c.query(`INSERT INTO notifications (tenant_id, user_id, type, title, body, entity_type, entity_id)
             VALUES ($1,$2,'sla_reminder',$3,$4,'ticket',$5)`, [ticket.tenant_id, ticket.assignee_id,
                ...
                const [assignee] = await db.withSuperAdmin(async (c) => {
                    const r = await c.query(`SELECT email, name FROM users WHERE id = $1 AND email IS NOT NULL`, [ticket.assignee_id]);
                    return r.rows;
                });
                if (assignee) {
...
<p><strong>${remMins > 0 ? `${remMins} minutes remaining` : 'SLA due very soon'}</strong> — please action this ticket now.</p>`,
                ...
                core_1.logger.info(`SLA reminder sent for ticket ${ticket.ticket_number}`);
            }
            ...
            if (pct >= l1Pct && ticket.escalation_level < 1 && !ticket.escalated_l1_at) {
                const managers = await db.withSuperAdmin(async (c) => {
                    const r = await c.query(`SELECT id FROM users WHERE tenant_id = $1 AND role IN ('manager','tenant_admin') AND is_active = true`, [ticket.tenant_id]);
                ...
                await db.withSuperAdmin(async (c) => {
                    await c.query(`UPDATE tickets SET escalation_level = 1, escalated_l1_at = NOW() WHERE id = $1`, [ticket.id]);
                    await c.query(`INSERT INTO ticket_escalations (tenant_id, ticket_id, escalation_level, reason, notified_users)
             VALUES ($1,$2,1,'sla_breach',$3)`, [ticket.tenant_id, ticket.id, notifyIds]);
                    for (const uid of notifyIds) {
                        await c.query(`INSERT INTO notifications (tenant_id, user_id, type, title, body, entity_type, entity_id)
               VALUES ($1,$2,'sla_breach',$3,$4,'ticket',$5)`, [ticket.tenant_id, uid,
                ...
                const l1Users = await db.withSuperAdmin(async (c) => {
                    const r = await c.query(`SELECT email, name FROM users WHERE id = ANY($1) AND email IS NOT NULL`, [notifyIds]);
                    return r.rows;
                });
...
<p>Please take immediate action to resolve or escalate this ticket.</p>`,
                ...
                core_1.logger.warn(`SLA L1 escalation for ticket ${ticket.ticket_number}`);
            }
            // ── L2 Escalation (hard) ─────────────────────────────────────────
            if (pct >= l2Pct && ticket.escalation_level < 2 && !ticket.escalated_l2_at) {
                const admins = await db.withSuperAdmin(async (c) => {
                    const r = await c.query(`SELECT id FROM users WHERE tenant_id = $1 AND role IN ('tenant_admin','super_admin') AND is_active = true`, [ticket.tenant_id]);
                    return r.rows.map((u) => u.id);
                });
                const overMins = Math.round((now - dueMs) / 60_000);
                await db.withSuperAdmin(async (c) => {
                    await c.query(`UPDATE tickets SET escalation_level = 2, escalated_l2_at = NOW() WHERE id = $1`, [ticket.id]);
                    await c.query(`INSERT INTO ticket_escalations (tenant_id, ticket_id, escalation_level, reason, notified_users)
             VALUES ($1,$2,2,'timeout_l2',$3)`, [ticket.tenant_id, ticket.id, admins]);
                    for (const uid of admins) {
                        await c.query(`INSERT INTO notifications (tenant_id, user_id, type, title, body, entity_type, entity_id)
               VALUES ($1,$2,'sla_escalated',$3,$4,'ticket',$5)`, [ticket.tenant_id, uid,
                ...
                const adminUsers = await db.withSuperAdmin(async (c) => {
                    const r = await c.query(`SELECT email, name FROM users WHERE id = ANY($1) AND email IS NOT NULL`, [admins]);
                    return r.rows;
                });
...
<p>Immediate intervention is required.</p>`,
                ...
                core_1.logger.error(`SLA L2 escalation for ticket ${ticket.ticket_number}`);
            }
        }
    }
    catch (err) {
        core_1.logger.error('[SLA Worker]', { error: err.message });
    }
}
// ── Platform Module ────────────────────────────────────────────────────────
class TicketingPlatformModule {
    id = 'ticketing';
    label = 'Ticketing';
    icon = 'LifeBuoy';
    ...
    navItems = [
        ...
        { path: '/tickets/queues', label: 'Queues', icon: 'List', permissionKey: 'tickets:read' },
        { path: '/tickets/sla', label: 'SLA Policies', icon: 'Clock', permissionKey: 'settings:read' },
    ];
    slaHandle;
    async onLoad(ctx) {
        ...
        core_1.logger.info(`Ticketing module loaded — SLA worker every ${SLA_WORKER_INTERVAL_MS / 1000}s`);
    }
    async onUnload() {
        if (this.slaHandle) {
            clearInterval(this.slaHandle);
            this.slaHandle = undefined;
        }
        core_1.logger.info('Ticketing module unloaded');
    }
    // Routes are registered directly in server.ts — nothing to do here
    async registerRoutes(_fastify, _prefix) { }
}
exports.TicketingPlatformModule = TicketingPlatformModule;
