...
type SmsConnectorConfig =
  ...
  | { provider: 'platform'; config: TwilioSmsConfig }
...
function getPlatformGatewayConfig(): TwilioSmsConfig | null {
...
export class SmsService {
  ...
  async send(tenantId: string, opts: SendSmsOpts): Promise<SendSmsResult> {
    ...
    try {
      const result = provider === 'twilio' || provider === 'platform'
        ? await this.sendViaTwilio(config as TwilioSmsConfig, opts)
        : await this.sendViaHttp(config as HttpSmsConfig, opts);
  ...
  private async recordPlatformGatewayUsage(tenantId: string): Promise<void> {
