      ...
      const [policy] = await db.withTenant(req.tenant.id, async (client) => {
        const r = await client.query(
           `INSERT INTO sla_policies
              (tenant_id, name, description, priority, first_response_hours, resolution_hours, time_unit,
           ...
           [req.tenant.id, body.name, body.description ?? null, body.priority,
            body.firstResponseHours, body.resolutionHours, body.timeUnit ?? 'hours',
      ...
      const [policy] = await db.withTenant(req.tenant.id, async (client) => {
        const r = await client.query(
           `UPDATE sla_policies SET
             ...
             time_unit            = COALESCE($6,  time_unit),
             reminder_pct         = COALESCE($7,  reminder_pct),
             l1_escalation_pct    = COALESCE($8,  l1_escalation_pct),
             l2_escalation_pct    = COALESCE($9,  l2_escalation_pct),
             business_hours_only     = COALESCE($10,       business_hours_only),
             business_hours_schedule = COALESCE($11::jsonb, business_hours_schedule),
             pause_on_pending        = COALESCE($12,       pause_on_pending),
             match_conditions        = COALESCE($13::jsonb, match_conditions),
             is_active               = COALESCE($14,       is_active),
             reminder_schedule       = COALESCE($15::jsonb, reminder_schedule),
             ticket_type             = COALESCE($17,       ticket_type),
             updated_at              = NOW()
           WHERE id = $16 AND tenant_id = $18 RETURNING *`,
          [body.name, body.description, body.priority, body.firstResponseHours,
           body.resolutionHours, body.timeUnit ?? null, body.reminderPct, body.l1EscalationPct,
