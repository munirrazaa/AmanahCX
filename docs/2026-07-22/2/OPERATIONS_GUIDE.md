...
Last updated: 2026-07-22 (2)

---

## 5 Real Problems Fixed This Week (2026-07-22)

Following up on the full test pass below, here's what actually got fixed and verified working:

1. **An agent could open a colleague's ticket from another department** just by knowing its reference number, and even change it. Now, if it's not yours, it's not reachable — same rule the main ticket list already followed, now applied everywhere.
2. **Turning Voice Bot off and back on didn't actually bring it back.** Traced to three small things stacked on top of each other, all fixed — toggling it now works instantly, both ways.
3. **Managers can now create their own new ticket queues** directly, without needing anyone else — previously this button existed but nobody could actually use it.
4. **Tenant admins can now delete a contact** when needed — previously this was technically "allowed" for nobody in practice.
5. **Deleting a company that still has a contact linked to it now works cleanly** instead of showing a confusing error — the contact stays exactly as it was, just no longer tied to that company.

We also updated the Operations Manuals for Tenant Admins and Managers to reflect items 2–4 above, and agreed on a longer-term plan (not built yet, tracked for later) to make it easier to hand out one specific ability to one specific person, instead of always being tied to a fixed role.

---

## Full System Test Pass — What Was Checked and What We Found (2026-07-22)

We went through the entire test checklist for the app — every module, every role, 188 checks in total. Most things (145 out of 188) work exactly as they should. Here's what that means for your team in plain language:

**Things we found and already fixed:**
- Opening a customer's full history (their past tickets and to-dos together in one place) was broken and always showed an error — it now works correctly.
- Filtering your contact list down to just the people who work at one company wasn't working — it now does.

**Things we found that still need attention (not fixed yet, on purpose — flagging for you rather than guessing at the fix):**
- The biggest one: right now, an agent can technically open and change a ticket that belongs to a completely different department, just by knowing (or guessing) its reference number. The normal ticket list correctly hides other departments' work — this is only about someone directly navigating to a ticket that isn't theirs.
- If you turn a feature (like Voice Bot) off for a workspace and then turn it back on, it doesn't actually come back until we step in manually — we've found exactly why and it's a fixable naming mismatch in the code, just needs a proper pass rather than a rushed patch.
- Nobody can currently create a brand-new ticket queue or delete a contact through the app — both are locked behind a permission combination that nobody actually has.
- A few smaller things: no way to filter tickets by date range yet, no way to see one agent's customer-satisfaction score on its own, and deleting a company that still has a contact linked to it shows a confusing error instead of a helpful one.

**Things we couldn't test yet, and why:**
- Email inbox and API/webhook features need a paid add-on that isn't turned on for any of our test workspaces yet.
- Voice call history/analytics need an actual phone call to have happened first — none of our test workspaces have made one yet.
- Creating a brand-new workspace from scratch wasn't tested this round, to avoid leaving extra test workspaces lying around without checking with you first.

Everything above is written up in full detail in the Bug Report and To-Do List documents, so nothing gets lost or forgotten.

---

## Every User Role Checked and Cleaned Up (2026-07-21)

**Who this affects:** everyone — but especially Operations Admins, Policy/Governance Admins, Collaborators,
and Viewers, whose dashboards were showing the wrong information before today.

**What was wrong:** we went through every single type of user in the system — Admin, Operations Admin,
Manager, Policy Admin, Agent, Collaborator, Viewer, and Super Admin — and clicked through everything they
can see, checking it actually made sense for that role. We found several real problems: two roles
(Operations Admin and Collaborator) couldn't even be assigned to anyone — the option to create them was
simply missing. Three roles were shown a confusing home screen meant for agents, with "my tickets" and
"my calls" boxes that would always show zero, since those roles are never assigned tickets or calls
directly. The "Viewer" role, whose entire purpose is read-only access, was in some cases quietly getting
full edit rights by mistake. And for regular agents, a setting had gotten stuck in a way that hid the
Tickets and Sector Field labels from view (e.g. showing generic "Contacts" instead of the correct
"Shoppers" for an online retail business).

**What changed:** all of the above is now fixed. Every role can be assigned properly, every dashboard shows
the right home screen for what that role can actually do, Viewer accounts are properly locked to read-only,
and the sector-specific labels are back. Two smaller, cosmetic labelling issues (module names showing as
"Crm" or "Voice_bot" instead of "CRM" or "Voice Bot") were also cleaned up in three places.

**Still to do:** the ticket-count numbers shown at the very top of the Tickets page don't yet correctly
filter for every role — some roles may see a total that includes tickets they can't actually open below it.
This needs a separate piece of backend work and is logged for a future session; it does not affect data
security, only what a summary number displays.

---

## Behind-the-Scenes Reliability Check — Fixed (2026-07-21)

**Who this affects:** nobody directly — this was an internal safety-net fix, not a visible feature change.

**What was wrong:** the system's internal code-quality checker had never actually been run properly across
most of the platform's backend, so it couldn't be trusted to catch real mistakes before they happened.

**What changed:** the checker now runs cleanly across the entire backend. While fixing it, we also found and
fixed two small, real bugs it had been hiding: emails sent from the platform weren't correctly recording
which staff member sent them, and one internal notification was silently not being delivered. Both are now
fixed. Nothing else about how the platform looks or behaves has changed for any user.
...
**What was wrong:** only one test workspace (and your real client, Haier Electronics) had the full set of industry-specific fields (things like policy numbers for insurance, tracking numbers for logistics, etc.) and a full Sales/Support/Complaints team to test with. Every other test workspace was missing most of those fields and only had one admin login — there was no way to actually test whether a ticket sent to "Sales" landed with the Sales team instead of Support.

**What's fixed:** every workspace now has its full set of industry fields, plus a Sales team, a Support team, a Complaints team, and a Field Sales team (a manager + an agent each) — all logged in and tested for real. Confirmed a ticket created for Sales is only visible to Sales staff, not Support or Complaints staff, and the right agent can pick it up and it stays assigned to them.

**Also fixed:** the app itself was occasionally crashing entirely when it had a brief hiccup talking to the database — one bad moment used to take the whole system down instead of just failing that one request. That's now handled gracefully; a hiccup no longer brings anything down.

**A new master password was set** for every test account (see the Credentials file) — the old one had had issues previously.

---

## Voice Bot Was Not Actually Using Each Workspace's Own Settings (fixed — 2026-07-18)

**Who uses it:** every workspace with the Voice Bot enabled.

**What was wrong:** the bot's per-workspace settings (company name, tone, script, rules) were being saved
correctly and looked correct in the CRM — but the bot itself was never actually able to read them at call
time. A technical permissions check meant to protect workspace data was also, unintentionally, blocking the
bot's own request for its own settings. Because of this, the bot was quietly using generic fallback
behavior on every real call, for every workspace, regardless of what had been configured.

**What's fixed now:** the bot correctly reads each workspace's real settings before every call — the right
company name, tone, and script now apply as configured.

**Tested:** confirmed directly that the bot's setting-lookup now returns Haier Electronics' actual
configuration instead of the generic fallback.

---

## Voice Bot: Call Capacity Limits, Human Handoff, and Agent Builder (new — 2026-07-18)

**Who uses it:** Super Admin builds and assigns bots; tenant admins see the result automatically.

**Call capacity limits — Super Admin → Config Ownership → Capacity.** You can now cap how many
calls the voice bot handles at once for a given workspace. If several workspaces share the same
server, this stops one busy workspace from crowding out the others during a rush of calls.

**When the bot can't take a call — minutes used up, or too busy.** Instead of just apologising and
asking the caller to wait for a callback, the bot now raises an urgent ticket immediately with
everything said so far in the call, so whoever follows up already knows what the caller needed —
no need to make them repeat themselves. If a live "transfer straight to a person" number has been
set up for that workspace, it will also try connecting the caller directly. (Live transfer needs
the real phone line connected first — not yet turned on until that's confirmed working.)

**Smarter recognition of returning callers.** Previously the bot only recognised a caller by phone
number. Now it checks two pieces of information together — phone, ID number, or email — before
treating someone as a known contact, which is much less likely to mix up two different people. If
a caller only gives one piece of information (common on a quick question), the bot will read back
their name and ask them to confirm it's really them before relying on it.

**Agent Builder — Super Admin → Agent Templates (new page).** Build a voice bot's personality once
— company name, tone, language, voice, ground rules — save it as a template, and assign it to any
workspace. Assigning it automatically turns on Voice Bot access for that workspace if it wasn't
already on. The Knowledge Base (the answers the bot can give directly, without a ticket) can now be
filled in three ways: type it, upload a file, or paste a website link — any combination.

**New workspaces get a working bot from day one.** If a new workspace is set up with Voice Bot
included, it now gets a bot immediately — greeting callers by the company's real name, and offering
exactly the right menu: general questions are always answered directly when possible, Complaints
appears if Ticketing is included, and Sales appears if the Sales module is included.

**Tested:** every piece above was tested using disposable test workspaces and test data, never
against a real client's account, with everything cleaned up afterward. Live transfer to a real
person is the one piece still waiting on the phone line itself being connected before it can be
proven on an actual call.

---

## Sales Calls Handled by the Voice Bot Now Reach Your Sales Pipeline (fixed — 2026-07-18)

**Who this affects:** Sales managers and agents at any workspace using the voice bot (Nadia).

**What was wrong:** When a customer called in with a sales enquiry and the voice bot handled it, the ticket
correctly landed in your Sales team's queue — but behind the scenes it was labeled as a general complaint
instead of a sales enquiry. Because "turn this into a pipeline deal automatically" only fires for tickets
labeled as sales, those voice-originated leads were quietly never turning into deals in your pipeline, even
though everything else about them looked correct.

**What's fixed:** The ticket now keeps the label it actually is. Sales enquiries from phone calls now flow
into the pipeline automatically the moment your agent accepts the ticket, exactly like ones entered manually.

## Manager Escalation Alerts Now Stay Within the Right Department (fixed — 2026-07-18)

**Who this affects:** Managers in any workspace with more than one department (e.g. Support, Sales,
Complaints).

**What was wrong:** When a ticket was about to breach (or breached) its SLA deadline, the alert was going out
to every manager and admin in the whole workspace — a Support ticket running late would also alert your Sales
and Complaints managers, which isn't useful to them and adds noise.

**What's fixed:** Breach alerts now go only to the agent's own manager, and if it's still not resolved, that
manager's manager — the same as how every other manager/agent visibility rule in the system already works.
...
## Sub-Admin Roles Now Actually Restrict Access (fixed — 2026-07-18)

**Who uses it:** Super Admin, and anyone given a delegated Sub-Admin Role.

**What was wrong:** in Super Admin → Sub-Admin Roles, you could tick specific checkboxes for a named role (e.g. give "Voice Bot Configurator" only the Voice Bot boxes) and it looked like it worked — but that only controlled which buttons showed on screen. Behind the scenes, the checkboxes weren't actually being checked, so nothing stopped that account from doing anything a full Super Admin could do if they knew the right web address. On top of that, sub-admin accounts couldn't even log in and reach the Super Admin area at all — that part of the feature was silently broken.

**What's fixed now:** a sub-admin account can log in and gets restricted for real — if their role only has Voice Bot boxes ticked, every other Super Admin action (managing tenants, billing, other roles, etc.) is refused with a clear "you don't have permission" message, not just hidden. The true Super Admin account (yours) is completely unaffected — full access as always.

**A few new permission boxes were added** to cover actions that didn't fit the existing list cleanly: managing tenant users/passwords, per-tenant voice bot usage & billing, the shared SMS/integrations settings, platform alerts, the metrics dashboard, other reports, and entitlement sync. If you had already built sub-admin roles before this fix, revisit them — anything relying on those new areas will need the new box ticked to keep working.

**Tested:** created a disposable throwaway role and test account, confirmed it could only do what its ticked box allowed and was refused everywhere else, then deleted the test account and role — no real tenant or account was touched.
