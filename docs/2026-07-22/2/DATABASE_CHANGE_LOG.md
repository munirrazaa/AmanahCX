...
## 2026-07-22 (2) — No schema changes; disposable test data cleaned up during bug-fix verification

### What changed
- No migrations run, no schema changes. All 5 bug fixes this push were application-code-only.
- Confirmed via direct database inspection (not just the app) that `sales_opportunities.company_id` already has `ON DELETE SET NULL` at the schema level (migration 018) — relevant to the company-deletion fix, no change needed to this constraint.
- Disposable test records created while verifying each fix (test tickets, contacts, companies, a temporary manager account, a temporary ticket queue) were all deleted immediately after each verification — no test data left behind on Haier Electronics.
- Existing staff permissions affected by the voice-permission bug (see Change Log) were NOT corrected in this push — that's a data fix on top of the code fix, and needs a decision on scope (just Haier, or a check across all 8 tenants) before touching real staff records.

### Why
- Routine bug-fix verification; no deliberate database work this push.

---

## 2026-07-22 — Test-plan execution: 1 mislabeled field corrected, disposable test data cleaned up, no schema changes

### What changed
- No migrations run, no schema changes. This was a testing pass (188-row test plan), not a build.
- One data fix on the `demo-education` tenant: two genuinely different custom fields shared the identical label "Course / Program" — relabeled one to "Currently Enrolled In" (same fix noted in the previous test pass, re-confirmed here).
- Haier Electronics' workspace name and module licensing were both temporarily changed during testing (name to "Haier Electronics QA TU", Voice Bot toggled off) and fully reverted to their original values before this push — confirmed via before/after comparison, no lasting change to production data.
- A second demo tenant ("Demo Company") had its module licensing temporarily changed during a module-assignment test and fully restored to its original 8-module set afterward — confirmed via before/after comparison.
- Disposable test records created during testing (contacts, companies, deals, tickets, activities, SLA policies, tags, departments, temporary staff accounts) were deleted/deactivated after each test where deletion was possible; a small number of test tickets/contacts remain in Haier Electronics with clear "QA ... TU" naming for traceability, consistent with this session's testing pattern.

### Why
- Routine test-plan execution; the one field relabel and the reverted tenant changes were incidental to testing, not deliberate database work.

---

## 2026-07-21 (2) — `platform_deploy_log` table + 14 new sector custom fields + 1 corrected user record

### What changed
- New migration `074_platform_deploy_log.sql`: `platform_deploy_log (id, triggered_by, targets TEXT[], results JSONB, created_at)` — audit trail for the Push-to-Production mechanism. Applied via the standard migration runner.
- 14 new custom field definitions added to `packages/shared/src/config/sectors.ts` across 7 sectors (banking, telecom, public transport, logistics, insurance, education, e-commerce) — these seed into `custom_field_definitions` for any tenant on those sectors going forward; existing tenants are unaffected until fields are explicitly re-synced.
- One data correction on Haier Electronics: an existing Customer Support agent's `users.permissions` JSON had `tickets:"none"` (contradicting their own assigned tickets) — corrected to match their identical peer teammate's permissions.
- One duplicate ticket tag removed on Haier Electronics ("billing issue" / "billing-issue"), and 2 stray test tickets (voice-bot-minutes-exhausted test artifacts) closed and permanently deleted — approved by user, Haier is a demo tenant, not a live customer.

### Why
- The deploy-log table backs the newly-built Push-to-Production audit trail.
- The sector field additions came out of comparing AmanahCX's per-sector field coverage against equivalent fields offered by top CRMs for the same industries.
- The permission correction and test-data cleanup came out of the 8-phase Dashboard Audit (see `CHANGE_LOG.md` same date).

### Impact
None on existing tenant data beyond the one corrected user record and the 3 removed test artifacts on Haier Electronics (all explicitly approved). The new table and field definitions are additive only.
