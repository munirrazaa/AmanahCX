...
## Change Log - 2026-07-22 (2) — 5 high/medium-risk bugs fixed and live-verified, Operations Manuals updated

### Fixed
- **HIGH PRIORITY — cross-department ticket access**: any agent could view/edit any other agent's ticket in a different department by direct ticket ID (the list correctly scoped visibility, single-ticket GET/PATCH/actions didn't). Added `assertTicketVisible()`, a shared guard applied to all 21 single-ticket routes (view, edit, assign, accept, resolve, close, escalate, comments, and 14 others). Caught and fixed a bug in the fix itself (a parameter-count mismatch that broke manager access) before confirming clean. Live-verified: exploit blocked (404), all legitimate access paths (own tickets, unassigned-queue claiming, manager oversight, tenant_admin observer) unaffected.
- **HIGH PRIORITY — module re-enable bug**: toggling a licensed module (Voice Bot) off then back on never restored it. Three stacked causes, all fixed together: (1) the module's registered id (`voice`) never matched its licensing key (`voice_bot`) anywhere else in the system; (2) `GET /api/v1/modules` never read the tenant admin's own enable/disable toggle at all, only the super admin's licensing ceiling; (3) the toggle's own write path never invalidated the 5-minute tenant cache. Also fixed a related bug in the same subsystem: the safety net that zeroes out unlicensed-module permissions used two module keys (`voice`, `voicebot`) that never matched the real key either, silently wiping voice permissions for every invited/updated user regardless of actual licensing — fixed for new/updated users; existing affected users still need a data correction (logged separately, scope needs a decision).
- **Nobody could create a ticket queue**: the "New Queue" button was gated to `manageWorkspace` (tenant_admin), but tenant_admin has read-only observer access to everything ticket-related — the one role that could see the button could never use it. Added a dedicated `manageQueues` permission (manager+), matching the existing SLA-policies exemption pattern. Live-verified as a manager: create and delete both work.
- **Nobody could delete a contact**: gated to tenant_admin/super_admin, but tenant_admin is separately blocked from all contact operations — unreachable by any role. Benchmarked against Zendesk/Salesforce/HubSpot/Freshdesk (all restrict this to admin-tier by default) before deciding: kept the admin-only intent, carved a narrow tenant_admin exception for this one action only (same pattern as the SLA-policies exemption). Live-verified: delete works; every other contact operation remains correctly blocked for tenant_admin.
- **Company deletion crashed with linked contacts**: no handling around the foreign-key safety check — any linked contact/deal/activity/ticket caused a raw 500. Chose auto-unlink (matches Zendesk/HubSpot's default; Salesforce's cascade-delete felt riskier) over block-with-message. Now nulls `company_id` on contacts/deals/activities/tickets before deleting; `sales_opportunities.company_id` already has `ON DELETE SET NULL` at the schema level, no manual query needed there. Live-verified: company deletes cleanly, linked contact survives intact with its company link correctly cleared.

### Also fixed along the way
- Removed 28 stale compiled `.js`/`.js.map`/`.d.ts` files checked into `modules/*/src/` directories across all 11 backend module packages — one of them was silently shadowing a live `.ts` fix during verification (tsx's directory-import resolution prefers `.js` when both exist). Added a `.gitignore` rule to prevent recurrence.

### Documentation
- All 4 Operations Manuals updated: Tenant Admin (module re-enable now reliable, contact-deletion exception explained) and Manager (new queue-creation capability documented) got real content additions; Agent and Viewer reviewed for drift, confirmed no changes needed.
- Agreed direction with the user for the underlying pattern behind the queue/contact-delete conflicts: a granular, assignable permissions system (matching how Salesforce/Zendesk Enterprise/HubSpot/Freshdesk handle this) instead of more one-off role-rank exceptions — logged to Backlog and Bug Report as a planned feature, not built this session.

---

## Change Log - 2026-07-22 — Full Test-Plan execution (188 rows), 2 real bugs fixed, 6 significant gaps found and logged

### Testing
- Ran through every row of `AmanahCX-Test-Plan.csv` still marked Pending — 188 total rows across 17 modules. Result: 145 Pass, 15 confirmed real issues ("No"), 28 genuinely blocked (billing-gated features, or needs approval to create test tenants/real phone calls — all logged to the To-Do list with the exact reason).
- Categories fully exercised this pass: Custom Fields, SLA Queues & Tags, Manager/Agent Ticket Handling, Contacts, Companies, Deals Pipeline, Activities, Analytics & Reports, Tenant Isolation (10/10, all clean), Settings & Notifications, Tenant Admin Modules, Departments & Users, Super Admin module-assignment (via an existing tenant, no new workspace created).

### Fixed
- **Contacts — linked activities/tickets tab was completely broken**: the `/contacts/:id/timeline` query mixed a jsonb column with a text column (`transcript` vs `metadata::text`) and referenced two columns that no longer exist (`assigned_to` instead of `assignee_id` on tickets, `stage_name` instead of `stage_id` on deals). Fixed all three; verified live — a contact's linked tickets/activities now display correctly.
- **Companies — filtering contacts by company was completely unimplemented**: added the missing `companyId` filter clause to the contacts list/count queries (`packages/api/src/routes/contacts.ts`), mirroring the existing owner-filter pattern. Fixes both the direct API filter and the Companies page's "linked contacts" tab, which relied on the same broken filter.

### Found, logged, not fixed (flagged for dedicated review per user instruction)
- **HIGH PRIORITY** — any agent can view AND edit any other agent's ticket in a different department just by its ticket ID; the ticket list correctly scopes visibility but the single-ticket view/edit endpoints only check tenant, not ownership.
- **HIGH PRIORITY** — toggling a licensed module off then back on doesn't restore it to the sidebar; root-caused to a real naming mismatch (`voice` vs `voice_bot`) between the module registry and the newer licensing system — confirmed via direct DB check and a full server restart (not a caching issue).
- Nobody, in any role, can create a new ticket queue (permission gap between tenant_admin's server-side block and other roles' missing UI permission).
- Nobody, in any role, can delete a contact (same permission-architecture pattern as the queue issue above).
- Deleting a company with a linked contact crashes with a generic error instead of a clear message (a database safety check is working correctly, but the app doesn't explain it — also raises a design question needing a decision).
- Smaller gaps: no date-range filter on Tickets; no per-agent CSAT filter; Education-sector ticket field names don't match the test plan (documentation drift, same pattern already known for Transport/Logistics).

### Also included in this push (pre-existing, unrelated work already sitting in the working tree)
- Line-manager dropdown fix in `AdminUsers.tsx` (Invite + Edit modals) — a department with only one manager tier now correctly falls back to showing that manager, instead of showing an empty dropdown.
- Sales Demo tool access tightened from `tenant_admin, super_admin` to `super_admin` only, since it's an internal AmanahCX sales tool, not a tenant-facing feature — route added to the frontend, hook renamed to `useIsSuperAdmin`.

---

## Change Log - 2026-07-21 (2) — Custom Fields improvements, Governance consolidation, Push-to-Production built, 8-phase role-by-role Dashboard Audit

### Added
**Push to Production is now a real, working mechanism (still parked pending 2 URLs)**
- Backend route `POST /super-admin/platform/redeploy`, permission gate, audit log table (`platform_deploy_log`, migration `074_platform_deploy_log.sql`), and a Settings panel with a confirm step are all built and live-tested. Reads `DEPLOY_HOOK_VERCEL_URL`/`DEPLOY_HOOK_RAILWAY_URL` from the server environment and calls each hook. Parked at user's request until those two URLs are created.

**Custom Fields: alphabetical ordering, sector-gap fields, Company custom fields end-to-end**
- New fields save in alphabetical position (backend `sort_order` midpoint calculation) instead of always landing at the bottom.
- Added 14 fields across 7 sectors that were missing versus top CRMs' equivalents (banking, telecom, public transport, logistics, insurance, education, e-commerce) — see `packages/shared/src/config/sectors.ts`.
- Company records can now have custom fields, matching Contacts/Tickets/Deals: backend accepts/merges `customFields` on create and edit (`companies.ts`), frontend renders the sector field form on the Companies page, "Companies" tab restored in the Custom Fields builder.

**Governance consolidated into one hub**
- "Data & Privacy Policies" and "SLA Policies" (previously two separate sidebar items) and "Milestones" (previously buried in General Settings) are now one "Governance" sidebar entry with three tabs. Fixed a real bug found in the process: the Data & Privacy page's edit-permission check only recognized `policy_admin`, while the backend already treated `tenant_admin` as an implicit bypass — frontend and backend had drifted out of sync.

### Fixed — 8-phase Dashboard Audit (every role, Haier Electronics/ecommerce)
Systematic click-through of every page as every role (tenant_admin → operations_admin → manager → policy_admin → agent → collaborator → viewer → super_admin), checking for functional bugs, confusing UI, and sector/role mismatches. Found and fixed:
- **`operations_admin` and `collaborator` were completely dead roles** — no way to create either anywhere (missing from the Invite modal and 2 backend zod schemas in `settings.ts`). Wired both into the invite/edit flow and `roles.ts`'s `SYSTEM_ROLES` list.
- **Three roles were mis-rendered on the dashboard** as a generic, agent-styled "my tickets/my calls" personal-activity view under a wrong "Viewer View" label (`operations_admin`, `policy_admin`, `collaborator`) — none of them are ever assigned tickets or calls. `Dashboard.tsx` now gives each its own accurate label and a proper read-only empty state instead.
- **Rank-vs-permission mismatch**: `useRole.ts`'s `useCan().writeRecords`/`viewAnalytics` used `rank >= ROLE_RANK.agent`, which wrongly granted write/analytics access to `policy_admin` (rank sits above agent for authority-ordering reasons only, but its actual permission set is view-only everywhere). Excluded explicitly.
- **`viewer` and `policy_admin` had no case in `defaultPermissions()`** (`settings.ts`) — both silently fell through to the `agent` default, meaning every viewer ever invited before today got full write permissions (`tickets:"full"`, `contacts:"full"`, `activities:"full"`) despite the role being named "Read-only access." Added correct cases for both.
- **`GET /api/v1/sector` was wrongly scoped to `admin:read`** (manager+) despite existing code comments stating it's meant for "agents + managers + admins." This silently broke sector-aware labelling ("Shoppers" vs "Contacts", sector badge, "Add Shopper" vs "Add Contact") for every agent, tenant-wide, not just on this tenant. Fixed to use the already-existing `readHandler` (viewer+).
- **One real user's permissions record was corrupted**: an existing Customer Support agent had `tickets:"none"` stored against their account, contradicting their own assigned tickets and their identical peer teammate. Corrected to match.
- **Raw module-key labels** ("Crm", "Voice_bot") fixed in four separate spots across `TenantAdminDashboard.tsx` and `SuperAdmin.tsx` (platform dashboard's Module Adoption chart, the Tenants list, and a second tenant table) — consolidated into one shared `formatModuleLabel()` helper instead of leaving three near-duplicate inline fixes.
- Contacts.tsx's empty-state "Create your first contact" link had no permission check at all — fixed to respect `can.writeRecords` and use the correct sector-aware label.
- All test/demo user accounts created to audit each role were deleted after verification; no data left behind on Haier Electronics.

### Confirmed but not fixed (logged for a dedicated backend pass)
- `GET /api/v1/tickets/stats` applies no visibility scoping for any role — always counts every tenant ticket regardless of who's asking, while the ticket list right below it is correctly scoped. Produces a visible contradiction for restricted roles (stat card says "12 Open", list says "No tickets found").
- `GET /api/v1/tickets/dashboard/team` scopes by the requesting user's manager-tree, not tenant-wide, so an `operations_admin` (a cross-department observer by design) with no direct reports sees all-zero stats instead of the whole org's data.
- Full detail and per-phase breakdown logged in `Desktop/AmanahCX/To do/AmanahCX-To-Do-List.html`.

---

## Change Log - 2026-07-21 — Monorepo TypeScript build fixed (packages/api, core, shared, all 11 modules)

### Fixed
**Full monorepo type-check now passes with zero errors, across every backend package and module**
- Done in a controlled, isolated way: a dedicated branch (`fix/monorepo-build-config`), a git safety tag (`pre-monorepo-fix-2026-07-20`) and a full solution backup zip taken first, verified with a clean `tsc --noEmit` on every affected package individually plus the root `turbo run build` pipeline, then fast-forward merged into `main` only once everything checked out.
- Root causes: all 11 `modules/*` packages had never had their own `tsconfig.json` (added one to each); `packages/api` and `packages/core` had a `rootDir` setting that conflicted with how they import each other's source files directly (removed); a few files had wrong import paths, a duplicate/shadowed method in `tenant.service.ts`, and a handful of genuinely mistyped values.
- Byproduct: three real, previously-invisible bugs were found and fixed, none of which the app's normal `tsx` runtime had ever surfaced. `routes/emails.ts` was always recording `null` for "who sent this email" (`req.user.id` doesn't exist — should be `req.user.sub`) and was publishing its activity-created event with the arguments in the wrong order, silently breaking that event's downstream routing; `routes/sales/invoices.ts` had the same wrong-field bug.
- Confirmed but explicitly not fixed (separate, out-of-scope feature gap): `/calls/:callId/stream`'s live-call-streaming WebSocket route has never actually been reachable — the `@fastify/websocket` plugin it depends on was never installed or registered anywhere in the project.
- Files: new `modules/{activities,analytics,billing,connectors,contacts,crm,deals,sales,ticketing,voice-module,voice}/tsconfig.json`; `packages/api/tsconfig.json`, `packages/core/tsconfig.json` (removed `rootDir`); `packages/core/src/{email.service.ts,sms.service.ts,tenant/tenant.service.ts,index.ts}`; `packages/api/src/{middlewares/auth.middleware.ts,server.ts,lib/file-storage.ts,routes/{emails.ts,sales/invoices.ts,governance.ts,voice.ts,voice-bot.ts,super-admin.ts}}`; `packages/shared/src/types/api.ts`; `modules/billing/src/payment.interface.ts`; `modules/ticketing/src/index.ts`.
- No schema/migration involved, no database changes. No frontend behavior changes — this pass covered only the backend packages and modules; `packages/frontend`'s own separate build errors remain (tracked in `BACKLOG.md`).

---

## Change Log - 2026-07-20 (2) — Handoff tickets route to Support/Inquiry, carry caller's number

### Fixed
- When the voice bot can't take a call (minutes exhausted / over capacity), the urgent follow-up ticket
  was created with category 'other', which keyword-defaulted into the Complaints queue (seen live:
  TKT-00071 auto-assigned to a Complaints agent). Nobody knows yet what the caller wanted, so these now
  route as `inquiry` → the Support/Inquiry team (product decision 2026-07-20). TKT-00071 itself moved to
  Customer Support, unassigned, for fresh pickup.
- The handoff ticket now includes the caller's phone number (from the SIP call's caller-ID attributes)
  so the follow-up team knows who to call back. Browser test calls have no number — those tickets keep
  showing an anonymous "Caller"; real phone calls will carry the number once the trunk is live.
- Verified via a simulated handoff ticket: lands in Customer Support as inquiry. Test data cleaned up.

### Files
`services/nadia-voice-agent/src/agent.py`.

---

## Change Log - 2026-07-19 (2) — Customer voice-bot access limited to name + greeting by default

### Changed
**Full Voice Bot configuration is now an allocated capability, not a default admin right**
- Product decision: out of the box, a workspace — including its own tenant admin — can change only the
  bot's NAME and GREETING MESSAGE. The complete configuration (tone, speed, voice, language, guardrails,
  recording, self-service, knowledge base, custom intents) requires the existing `voicebot:configure`
  role permission, which the platform allocates per workspace (Super Admin → Manage Role Permissions →
  Admin role → Voice Bot). Once allocated, the tenant admin can delegate it to their own users via Roles.
- Enforced server-side on every tenant-side voice-bot write: `PUT /config` runs a limited
  name+greeting-only UPDATE for unauthorised users (403 `VOICEBOT_CONFIGURE_REQUIRED` if other fields are
  sent); knowledge-base and custom-intent writes are refused outright. Checked live against the roles
  table (custom role first, then the tenant's system-role ceiling), so an allocation/withdrawal takes
  effect without re-login. `GET /config` now returns `canConfigure`; the tenant page shows a limited-access
  banner and sends only the allowed fields when restricted.
- New tenants: the seeded Admin role now defaults `voicebot:configure` to false ("only if allocated").
  Existing tenants keep their current ceilings (Haier's admin remains allocated for ongoing testing).
- Live-tested as Haier's admin with the capability withdrawn: full save → 403; name+greeting-only save →
  success with no other fields touched; KB add → 403; capability restored → everything works again.

### Fixed
**Super Admin partial voice-bot saves no longer reset unspecified fields**
- See Bug Report 2026-07-19: the per-tenant upsert substituted defaults for missing fields and wrote them
  (a hold-message-only save reset tone from 'friendly' to 'professional'). Unspecified fields now preserve
  their existing values; defaults apply only on first-time creation. Verified live.

### Docs
- Voice Bot manuals (Desktop/AmanahCX/Voice Bot Manuals/): customer manual rewritten around the
  name+greeting default and the allocation/delegation flow; queue wording corrected (complaint tickets →
  Complaints queue, inquiries → Support/Inquiry queue, sales → Sales queue); Super Admin manual gained the
  allocation walkthrough. HTML + PDF copies in the folder root and the dated 2026-07-19/1 archive.

### Files
`packages/api/src/routes/voice-bot.ts`, `packages/api/src/routes/roles.ts`,
`packages/api/src/routes/super-admin.ts`, `packages/frontend/src/pages/VoiceBotConfig.tsx`.

---

## Change Log - 2026-07-19 — Configurable Hold Experience (message + branded audio)

### Added
**Editable hold message (no more code deploys for wording)**
- The line the bot speaks while creating a ticket ("please wait while I create your ticket") was
  hardcoded in `agent.py`; it's now a per-tenant `hold_message` field, editable in Super Admin →
  Config Ownership → Hold Experience, and on Agent Templates (copied on assign). Falls back to the
  built-in default when blank.

**Branded hold audio (e.g. a bank's product jingle)**
- Super Admin can upload an audio clip (mp3/wav/ogg/m4a, ≤5 MB) per tenant. While the bot does
  back-office work mid-call, the caller hears the clip instead of the spoken hold line; the clip
  stops the instant the bot is ready to speak again (LiveKit `BackgroundAudioPlayer`, stopped via
  its play-handle in a `finally`). Clip bytes live in the DB (`voice_bot_hold_audio`); Nadia
  fetches once per call via the shared-secret `GET /livekit/hold-audio` route and caches locally.
  Any failure in audio setup/playback silently falls back to the spoken line — never breaks a call.

### Live-tested
Upload → config round-trip → byte-identical fetch through Nadia's own connector → `load_settings()`
returns the hold message + audio flag. Audible behaviour (clip plays, stops on reply) still needs an
ear test on the next browser test call — a placeholder 2-second tone is currently uploaded on Haier
Electronics for exactly that; replace or Remove it in Config Ownership → Hold Experience.

### Files
Migration `073_voice_bot_hold_experience.sql`; `packages/api/src/routes/super-admin.ts`,
`packages/api/src/routes/voice-bot.ts`; `services/nadia-voice-agent/src/{agent,config,crm_client}.py`;
`packages/frontend/src/pages/SuperAdmin.tsx`.

---

## Change Log - 2026-07-18 — Generic three-flow bot for Haier + routing trusts the AI's classification

### Added
**Haier Electronics: generic inquiry / complaint / sales bot (via Agent Builder)**
- Updated the "Electronics Retail Support & Sales" agent template to an explicit three-flow script and
  re-assigned it to Haier Electronics: (1) **Inquiry** — product/policy questions answered directly from
  the knowledge base, no ticket unless the KB has no answer; (2) **Complaint** — collected empathetically
  and raised as a `complaint` ticket; (3) **Sales** — purchase interest captured as a `sales` ticket with
  the caller told a sales agent will call back; the bot never quotes prices/stock/discounts itself.
- Created a real **Sales queue** for Haier (push-assignment enabled) and linked the voice bot's sales menu
  option to it — sales tickets now land in the Sales queue (auto-assigning once Haier has sales agents;
  today they wait there for manual pickup since the whole team is Customer Support).

### Fixed
**Ticket routing now trusts the AI's explicit classification**
- The sales/inquiry/complaint router keyword-scanned the entire call text — so a COMPLAINT about a
  "product" contained the word "product" and got flipped into the inquiry queue. When the AI's category is
  exactly `sales`/`inquiry`/`complaint`, that decision is now used directly; keyword scanning remains only
  as the fallback for legacy/sector-specific category keys.
- Live-tested all three categories: complaint-with-product-wording stays a complaint; sales lands in the
  Sales queue; inquiry stays inquiry. Test tickets/calls/contacts deleted after.

### Files
`packages/api/src/routes/voice-bot.ts`; template + queue + IVR-link are data changes (no migration).

---

## Change Log - 2026-07-18 — Critical fix: Nadia's config fetch was silently blocked

### Fixed
**Every tenant's per-call bot config was being ignored — Nadia was always running hardcoded defaults**
- Nadia's config fetch (`crm_client.py get_config()`) called the tenant-side `GET /api/v1/voice-bot/config`
  route, which requires a real logged-in user and sits behind the global tenant-auth wall. Nadia has
  neither, so the request was silently rejected and fell back to `AgentSettings()` defaults —
  `bot_name="Nadia"`, the hardcoded HBL Microfinance Bank system prompt — for **every tenant**, the entire
  time this session's Agent Builder/config work was being built and believed live-verified.
- Discovered while confirming a newly-built Electronics Retail agent template actually reached a real call
  for Haier Electronics — it didn't; Nadia was still loading the banking script underneath despite the CRM
  showing the correct config.
- Fixed by adding `GET /api/v1/voice-bot/livekit/config` (same public, shared-secret pattern as the other
  `/livekit/*` routes) and pointing `crm_client.py` at it instead. Live-tested by calling Nadia's real
  `load_settings()` directly — confirmed it now returns the tenant's actual bot_name/tone/language/prompt.

**Agent template assign endpoint used the template's own name as bot_name**
- When a template had no explicit company-name override, assigning it set the tenant's `bot_name` to the
  TEMPLATE's generic name (e.g. "Electronics Retail Support & Sales") instead of the tenant's real name.
  Fixed to fall back to the tenant's actual name.

**Built: real Electronics Retail agent template, assigned to Haier Electronics**
- Replaced Haier's previously-empty system prompt (silently defaulting to the HBL banking script) with a
  proper electronics-retailer script covering product issues, warranty claims, delivery issues, sales
  inquiries, and FAQ-first handling for general questions.

### Files
`packages/api/src/routes/voice-bot.ts`, `packages/api/src/routes/super-admin.ts`,
`services/nadia-voice-agent/src/crm_client.py`.

---

## Change Log - 2026-07-18 — Doc process correction (Developer Orientation)

### Corrected
- The previous entry below claimed the Developer Orientation doc's uncommitted bug-narrative
  callout was correctly "restored." That was wrong — the deletion had been intentional: bug/issue
  narratives belong exclusively in the standalone Bug Report
  (`Desktop/AmanahCX/Bugs/AmanahCX-Bug-Report.html`), never in the Developer Orientation doc, which
  only describes current, resolved-state features. Removed the callout again.
- The Developer Orientation doc (html + pdf) was also being kept in two places — committed in this
  repo's `docs/` folder, and archived under `Desktop/AmanahCX/Orientation for Developers/<date>/<n>/`.
  Going forward it lives **only** in the dated Desktop archive folder; the repo copy has been deleted
  and will not be recommitted.

---

## Change Log - 2026-07-18 — Voice Bot: capacity/handoff, CRM-agnostic connector, smarter caller ID, Agent Builder

### Added

**Concurrent-call limits (per tenant + per server)**
- Super Admin can cap how many calls Nadia handles at once for a given tenant (Config Ownership →
  Capacity). A server-wide cap protects multiple tenants sharing one box from crowding each other
  out, plus a CPU-load-based worker ceiling as a hardware-level backstop.
- New `active_voice_calls` registry tracks in-progress calls in real time so both caps are enforced
  live, not just at config-save time.
- Live-tested: simulated overflow at both the per-tenant and global cap, confirmed correct rejection
  and cleanup on call-end.

**Human handoff with live transfer (when Nadia can't take a call herself)**
- Previously: minutes-exhausted / over-capacity calls only got an apology + callback ticket.
- Now: Nadia raises an urgent ticket FIRST with the full transcript-so-far (so whoever picks up
  already has context — no re-asking the caller), then attempts a live SIP transfer to a
  per-tenant configurable `human_transfer_destination`. Falls back to the original apology-and-end
  behaviour if no destination is set yet or the transfer fails.
- Live transfer itself cannot be proven until Telecard's SIP trunk is connected — the ticket +
  fallback path is fully live-tested; the transfer call (`ctx.transfer_sip_participant`) is
  code-complete but unverified on a real call.

**CRM Connector — Nadia is no longer AmanahCX-specific**
- Extracted the ~6 places Nadia's code called AmanahCX's specific API routes into one module,
  `crm_client.py`, behind a fixed 6-operation contract (get config, create ticket, search
  knowledge base, look up a caller, register a call, report call-ended).
- A different CRM (Salesforce, HubSpot, Dynamics, SugarCRM, etc.) can now plug into Nadia by
  implementing the same 6-method adapter — no changes to Nadia's voice/AI/SIP code required.
  AmanahCX is the reference implementation, not a special case.
- Documented in `services/nadia-voice-agent/CRM_CONNECTOR_CONTRACT.md`.
- Verified this was a behaviour-neutral refactor (diffed old vs. new request shapes — identical)
  before live-testing 3 of 6 operations against the real API.

**Smarter returning-caller recognition (two-of-three identity matching)**
- Previously matched contacts by phone number alone. Now requires two of {phone, NIC, email} to
  agree with an existing contact for a confident match — a coincidence/typo is far less likely to
  cause a false match. If only one identifier is available (e.g. a quick FAQ call), it's still
  matched but flagged "weak."
- New `identify_caller` tool lets Nadia check identity the moment a caller shares any ONE
  identifier — not only at ticket-creation time — reading back the first name and asking the
  caller to confirm before relying on a weak match.
- Live-tested: strong match (phone+NIC), weak match (phone-only, email-only), no-match on an
  unrelated number, and a follow-up call with a changed phone correctly re-matched via NIC+email
  without overwriting the original phone on file.

**Voice Bot Agent Builder (3 phases)**
- *Phase 1 — Agent Templates*: reusable voice-bot "recipes" (name, sector, company name,
  department, tone, character, language, guardrails, voice, call direction) created once in Super
  Admin and assigned to any workspace — not configured per-tenant from scratch each time.
  Assigning is a one-time copy into that tenant's real `voice_bot_configs` row (the same one Nadia
  reads at call time), so it stays editable afterward.
- *Phase 2 — Multi-source knowledge base*: Super Admin's knowledge base editor gained the same
  3-way source picker the tenant-side page already had — Add Text, Add Web Page (crawls and
  extracts text), Upload File (PDF/DOCX, text extracted server-side).
- *Phase 3 — Delegatable role*: added "Voice Bot" as a module in the Sub-Admin Roles permission
  matrix, so this whole area can be handed to a named role instead of staying Super-Admin-only.
- **Auto-grant fix**: assigning a template to a tenant not yet licensed for Voice Bot now
  auto-grants the module (+ Nadia/LiveKit provider feature) as part of the same action, instead of
  silently leaving a configured-but-unlicensed bot.
- **Auto-provisioning on tenant creation**: a new tenant licensed for Voice Bot now gets a working
  bot immediately — sector-matched template if one exists, generic defaults otherwise, greeting
  auto-filled with the real company name, and menu segments (Inquiry always on, Complaint/Sales)
  derived from which modules were actually licensed.
- Live-tested every piece via disposable test tenants/templates (never against real client data,
  after one testing mistake mid-session — see below); all test data cleaned up after each pass.

### Fixed (self-caught during testing, not a user-reported bug)
- Testing the Phase 1 assign endpoint was mistakenly run against the real Haier Electronics tenant
  instead of a disposable one, briefly overwriting its greeting message, tone, language, and
  system prompt with test values. Caught and restored the original values (Urdu, professional
  tone, no greeting override, original guardrails) before finishing. No lasting impact; logged here
  per the standing "every bug found gets a row" rule even though self-caught, not user-reported.

### Files
`services/nadia-voice-agent/src/agent.py`, `services/nadia-voice-agent/src/config.py`,
`services/nadia-voice-agent/src/crm_client.py` (new), `services/nadia-voice-agent/CRM_CONNECTOR_CONTRACT.md` (new),
`packages/api/src/routes/voice-bot.ts`, `packages/api/src/routes/super-admin.ts`,
`packages/frontend/src/pages/SuperAdmin.tsx`, `packages/frontend/src/App.tsx`,
migrations `064`–`066`, `072`.

---

## Change Log - 2026-07-18 — Voice-bot ticket-type bug + cross-department SLA escalation leak, both fixed

### Context
Found during a line-by-line audit of the "end-to-end information flow" diagram in the new Developer
Orientation document against the actual code — not a user bug report, but the same standard: every claim
checked against real code before being written down as fact.

### Fixed — Nadia voice-bot tickets always saved as `ticket_type='complaint'`
`createComplaintFromStructured` (`packages/api/src/routes/voice-bot.ts`) correctly classifies a call as
sales/inquiry/complaint by keyword-matching category+subject+description+transcript, and uses that
classification to route the ticket to the right queue — but the `ticket_type` column itself was hardcoded to
the literal string `'complaint'` in the INSERT, never the computed value. Since the accept-time
auto-conversion to a pipeline deal explicitly checks `ticket.ticket_type === 'sales'`
(`packages/api/src/routes/tickets.ts`), a sales enquiry captured by voice routed to the Sales queue correctly
but could never become a pipeline deal. Manually-created sales tickets were unaffected. Fixed to persist the
actual computed type.

### Fixed — SLA breach escalation ignored department isolation
`runSlaWorker` (`packages/api/src/routes/tickets.ts`) notified **every** active `manager`/`tenant_admin` in
the tenant on an SLA breach, regardless of department — a Support ticket breach would also page Sales and
Complaints managers. This broke the department-isolation model the rest of the system already enforces via
the reporting tree (`lib/visibility.ts`'s `getVisibleUserIds`). Added `getEscalationTargets()`: L1 now
notifies only the ticket assignee's own line manager, L2 only that manager's manager — walking
`users.manager_id` up from the agent, the same tree the rest of the app walks down from a manager. Falls back
to tenant_admin only when an agent has no manager configured, so a breach is never silently unnoticed.
Applied consistently across all four call sites that previously broadcast tenant-wide (first-response breach,
the configurable reminder schedule's "managers" target, and both L1/L2 resolution-SLA escalation).

### Verified
Both files typecheck clean (`tsc --noEmit`). No schema/migration change — pure application-logic fix, safe to
deploy without a database migration.

### Also updated
`docs/AmanahCX-Developer-Orientation.html` (developer/partner onboarding doc) — the end-to-end flow diagram
and its audit callout now describe the corrected behavior, with both findings documented inline as an
example of the doc being checked against real code rather than asserted from memory.
...
**Entitlement drift bug — 7 tenants were silently blocked from Analytics/Webhooks/Voice-Bot calling despite being licensed**
- The stale `settings.features.<name>` flag (documented as a known drift risk in `MASTER_PRODUCT_DOCUMENT.md` §2.7 / `BACKLOG.md` item 0b) turned out to still be actively enforced in exactly 3 places: `analytics.ts`, `webhooks.ts`, `voice.ts`. Checked live data across all 24 tenants — 7 were correctly licensed (`active_modules` says yes) but had never had the separate flag set, so they were 402'd on every request to those routes.
- Replaced all 3 with `requireModule()`, the one actively-maintained source of truth. Verified live: an agent in a previously-blocked tenant now gets a real 200 with data instead of `FEATURE_NOT_AVAILABLE`.

### Added

**Shared platform SMS gateway (opt-in per tenant)**
- Previously every tenant had to bring their own SMS gateway account — no shared AmanahCX-operated option existed at all. Added one: Super Admin can now flip "Enable shared SMS gateway" per tenant (Config Ownership panel). When enabled, `SmsService` automatically falls back to AmanahCX's own account (env-configured) whenever the tenant hasn't set up their own connector — zero setup needed on the tenant's end.
- Usage is metered per tenant per month (`usage_metrics`, metric `sms_sent_platform`) for future billing.
- Requires a real provider account (Twilio or a Pakistani aggregator) before it can send anything live — env vars documented in `.env`/`.env.example` (`PLATFORM_SMS_ACCOUNT_SID/AUTH_TOKEN/FROM_NUMBER`), currently unset so this is a safe no-op until filled in.
- Caught and fixed a bug in my own first draft: Postgres's `jsonb_set` doesn't create missing intermediate JSON objects, so the very first version of the toggle silently failed to save. Found by checking the database directly rather than trusting the success response.

**Voice bot cost tracking — new**
- `voice_bot_quotas` gains `cost_per_minute` (migration 070), settable per tenant by Super Admin.
- New `GET /super-admin/voice-bot/cost-report?month=YYYY-MM` — cross-tenant monthly cost, computed live from call duration × rate (mirrors the existing "never store a running total" pattern minutes already use).
- New Reports → Voice Bot Cost tab. Live-verified against Haier Electronics' real historical call data.

**Nadia — hold-message during ticket creation**
- `raise_ticket`'s ~2s CRM API call previously left the caller in silence. Now plays a short "one moment, I'm recording this" line (non-blocking — starts while the API call is in flight, not before it) so the line doesn't go dead. Code verified to compile; **not yet verified by ear** — no real phone call has been made against this pipeline yet (see the standing Nadia SIP trunk test checklist). Should be the first thing checked in that pass.

### Investigated, not changed

**Nadia streaming-speed / reply-pause coupling — confirmed still open, left alone on purpose**
- Re-checked the actual TTS code: it only avoids buffering the whole reply before playback when a tenant uses the *default* speaking rate. Any tenant with a custom rate still pays the full delay described in the original backlog note — this was NOT fixed despite an earlier research pass suggesting otherwise (corrected that here). Deliberately did not attempt a DSP-level fix blind — there's no way to verify audio quality (clicking, pitch artifacts) without hearing it, and this pipeline hasn't been tested on a real call yet at all. Stays open until it can be tested by ear.

**Mobile app offline scanner — already built, nothing to do**
- Confirmed via code read: `@react-native-ml-kit/text-recognition` on-device OCR is already the offline fallback for the business-card scanner, with graceful online/offline handoff and offline-queue sync already wired up (`packages/mobile/src/lib/cardOcr.ts`, `offlineQueue.ts`). Removed from the open list.

**Nadia standalone-product spin-off** — not a code task; needs a business/architecture scoping conversation (pricing, multi-tenant isolation, billing) before any implementation starts.
...
### Fixed
**Sector custom fields — 7 of 8 tenants were missing most of their fields**
- Only Haier Electronics had the full contact/company/deal/ticket field set for its sector. The other 7 tenants were missing most or all company/deal/ticket fields — the sector config (`packages/shared/src/config/sectors.ts`) had been expanded over time and nothing ever backfilled existing tenants.
- Re-ran the seeding logic against every tenant's current sector config (idempotent) — inserted 183 missing fields. All 8 tenants now verified to match their sector config exactly.

**API server crashing on transient database network blips**
- A single socket-level error on a pg connection mid-query (e.g. a Supabase pooler hiccup) was crashing the entire API process, not just the one request — `pool.on('error')` only covers idle pooled connections, not ones mid-query. Hit twice during live testing today.
- Added top-level `uncaughtException`/`unhandledRejection` handlers in `packages/api/src/server.ts` that log and keep the process running. Verified: server survived the rest of the test session with no further crashes.

### Added
**Full department/queue/agent test coverage across every tenant**
- Built out Sales/Support/Complaints (manager+agent) + Field Sales (Field Manager/Field Officer) users in the 6 single-admin demo tenants, plus the missing Sales/Complaints/Field Sales roles for Haier Electronics (its existing Customer Support staff left untouched). Every new agent/manager wired into their department's ticket queue.
- Universal test password reset for all 75 users across all 8 tenants (`documents/TEST-CREDENTIALS.md`, `Desktop/AmanahCX/Credentials/AmanahCX-Credentials.html`).
- Live-tested end-to-end across all 8 tenants: ticket created against a department's queue → correct department agent can see/accept it, wrong-department agent correctly cannot. Confirmed Haier's push-based (auto-assign) queues route correctly to the right department's online member, distinct from the pull-based queues used elsewhere.
- Also fixed: the pre-existing `demo` tenant's own staff had never been linked to their department queues (only the newly-built tenants got that wiring in the first pass).

### Also found (Sub-Admin Roles follow-up, not yet actioned)
- `platform:push` ("Push to Production") permission checkbox in Sub-Admin Roles is still cosmetic — no backend route or deploy credential exists yet. Tracked as an open item pending product-owner decisions on mechanism + credentials.

---

## Change Log - 2026-07-18 (Sub-Admin Roles — Real Server-Side Enforcement)

### Fixed
**Sub-Admin Roles permission matrix is now actually enforced by the API, not just the frontend**
- Previously: `/super-admin/*` was gated by a single `requireRole('super_admin')` check. `platform_role_id` and the Sub-Admin Roles permission matrix were saved to the database but never read by any route handler — the checkboxes only hid/showed buttons in the UI. Also discovered: sub-admin accounts (`role = 'platform_admin'`) were entirely blocked from every `/super-admin/*` route by that same check, so delegated sub-admin login didn't work at all.
- Now: the base gate accepts `super_admin` or `platform_admin`. Every one of the ~65 `/super-admin/*` routes has a new `requirePlatformPermission(key)` preHandler that checks the caller's actual permission matrix (loaded into the JWT at login from `platform_roles.permissions`). A true super admin (no `platform_role_id` assigned) is unrestricted; anyone with a `platform_role_id` — including a `super_admin` account that was assigned one — is limited to exactly what their role's matrix grants. Unlike tenant-role permission checks, an absent key here defaults to **deny**, not allow.
- Added new permission keys (frontend `PLATFORM_MODULE_DEFS` + backend) for routes that didn't map cleanly to the existing matrix: `tenants:manage_users`, `voice_bot:manage_tenants`, `integrations:manage`, `alerts:manage`, `metrics:view`, `reports:view`, `entitlements:sync`.
- Live-tested against the real login flow: created a disposable test platform role (only `voice_bot:manage_agents` granted) + a disposable test sub-admin user in a real tenant, logged in for real, confirmed the resulting JWT enforces exactly that one permission — allowed on Agent Templates, denied with a clean 403 on tenant listing, tenant suspend, and every other module. Confirmed the real super admin account is unaffected. All test data deleted after.
- Files: `packages/api/src/middlewares/auth.middleware.ts` (new `requirePlatformPermission` guard), `packages/api/src/routes/auth.ts` (both login paths now embed `platformRoleId`/`platformPermissions` in the JWT), `packages/api/src/routes/super-admin.ts` (base gate + per-route preHandlers), `packages/frontend/src/pages/SuperAdmin.tsx` (new permission keys), `packages/shared/src/types/user.ts` (`AuthToken` fields).
