...
### 0h. Cross-department ticket access via direct ID — FIXED
- **Verdict:** Done ✅ — 2026-07-22. Added `assertTicketVisible()` applied to all 21 single-ticket routes
  in `tickets.ts` (view, edit, assign, accept, resolve, close, escalate, comments, and 14 others), mirroring
  the list endpoint's existing visibility rules. Caught and fixed a parameter-count bug in the fix itself
  (broke manager access) before confirming clean. Live-verified: exploit blocked (404), all legitimate
  access (own tickets, unassigned-queue claiming, manager oversight, tenant_admin observer) unaffected.

### 0g. Module re-enable doesn't restore access — FIXED
- **Verdict:** Done ✅ — 2026-07-22. Three stacked causes, all fixed: (1) module registry id (`voice`)
  renamed to match the licensing key (`voice_bot`) everywhere else; (2) `GET /api/v1/modules` now
  intersects the super admin's licensing ceiling with the tenant admin's own enable/disable toggle
  (previously only read the ceiling, ignoring the toggle completely); (3) the toggle's write path now
  invalidates the 5-minute tenant cache immediately. Also fixed a related bug in the same subsystem: the
  licensing-enforcement safety net (`applyModuleLicensing`) used two wrong module keys (`voice`,
  `voicebot`) that never matched `voice_bot` either, silently zeroing voice permissions for every
  invited/updated user — fixed for new/updated users; **existing affected users still need a one-time
  data correction, scope (which tenants) not yet decided — logged as its own follow-up, not done here.**
  Live-verified end to end: toggle off removes the module, toggle on restores it, survives a full server
  restart, confirmed for a fresh invite.

### 0i. Nobody could create a ticket queue, or delete a contact — FIXED
- **Verdict:** Done ✅ — 2026-07-22. Queues: added a dedicated `manageQueues` permission (manager+),
  matching the existing SLA-policies exemption — the button was gated to tenant_admin, who is separately
  blocked from all ticket writes. Contacts: benchmarked against Zendesk/Salesforce/HubSpot/Freshdesk first
  (all restrict deletion to admin-tier by default) before deciding — kept the original admin-only intent
  and carved a narrow tenant_admin exception for this one action, rather than widening to manager.
  Both live-verified working end to end.

### 0j. Company deletion crashed with linked contacts — FIXED
- **Verdict:** Done ✅ — 2026-07-22. Chose auto-unlink over block-with-message (matches Zendesk/HubSpot's
  default; Salesforce's cascade-delete felt riskier). Nulls `company_id` on contacts/deals/activities/
  tickets before deleting the company; `sales_opportunities.company_id` already had `ON DELETE SET NULL`
  at the schema level, no manual query needed there. Live-verified: company deletes cleanly, linked
  contact survives intact with its company link correctly cleared.

### 0k. Granular, assignable permissions system — ROADMAP (not built)
- **Verdict:** Agreed direction with the user 2026-07-22, deliberately not built this session. Both 0i
  fixes above happened because a fixed role bundles all-or-nothing abilities — no way today to grant one
  person one specific extra capability (e.g. "can delete contacts") without either changing their whole
  role or a one-off code exception each time this conflict recurs.
- **Fix (future, dedicated session):** Build a capability-toggle system matching how Salesforce, Zendesk
  (Enterprise), HubSpot, and Freshdesk all handle this — specific permissions switched on/off per role or
  person, independent of the fixed role hierarchy. This app already has a "custom roles" foundation
  (Roles & Permissions screen) to extend rather than replace. Scope: every role's default permission set,
  the login-token/permission shape, and every screen/route currently checking a fixed role instead of an
  individual permission. Not a quick fix — needs its own design and testing pass.
...
### Nobody can create a new ticket queue, or delete a contact — permission gaps
- **Verdict:** Confirmed 2026-07-22, both same root pattern. Ticket queue creation: the "New Queue" button
  only shows for tenant_admin, but tenant_admin is server-side blocked from all ticket writes — so the one
  role that can see the button can't use it, and nobody else can see it. Contact deletion: gated to
  tenant_admin/super_admin, but tenant_admin is blocked from operational writes and super_admin has zero
  workspace access — so it's unreachable by any role.
- **Fix:** Needs a decision on who should hold these permissions (likely manager, or a narrow tenant_admin
  exception for these two specific actions) before implementing.

### Deleting a company with linked contacts crashes instead of a clear message
- **Verdict:** Confirmed 2026-07-22. The database correctly blocks the delete (foreign-key safety check),
  but the app surfaces a raw 500 error instead of explaining why.
- **Fix:** Needs a decision — auto-unlink the contacts and proceed, or block with a friendly "N contacts
  are linked, reassign them first" message. Either is a small fix once decided.
...
- **Status:** DONE for `packages/api`, `packages/core`, `packages/shared`, and all 11 `modules/*` — 2026-07-21 (commit 455046e, merged to `main`). Only `packages/frontend`'s own separate build errors remain (React Query v5 mismatches, missing type fields, `ImportMeta.env`) — logged as its own item below.
- **Why (original):** `npm run build` failed on pre-existing type errors across `@crm/core`, `@crm/api`, and every `modules/*` package (none had ever been individually type-checked before — production runs via `tsx`, which skips type-checking entirely). Deploys weren't blocked (Vercel runs `vite build` directly; Railway also skips the failing step) — but with the checker permanently red, a *real* new mistake could hide in the noise.

### Frontend TypeScript build errors (separate from the above, not yet fixed)
- **Verdict:** Do Soon (hygiene, not urgent)
- **Why:** `packages/frontend`'s own `tsc` run still fails — React Query v5 API mismatches, a few missing type fields, and `ImportMeta.env` typing. Out of scope for the 2026-07-21 monorepo build fix (that pass covered only the backend packages/modules). Doesn't block deploys (Vercel builds via `vite build`, not `tsc`).
...
- Sector custom fields backfilled tenant-wide (7/8 tenants were missing most fields) — DONE 2026-07-20
- Sales/Support/Complaints + Field Sales departments and users built out across all 8 tenants, wired into ticket routing queues, live-tested end-to-end — DONE 2026-07-20
- API process crashing on transient DB network blips — DONE 2026-07-20 (uncaughtException/unhandledRejection handlers added)
- [P2] `platform:push` (Sub-Admin Roles "Push to Production") permission is cosmetic, not wired to any real deploy mechanism — needs product-owner decision on mechanism + credentials before building
- Monorepo TypeScript build (`packages/api`, `packages/core`, `packages/shared`, all 11 `modules/*`) — DONE 2026-07-21 (commit 455046e); found and fixed 3 real bugs along the way (email `sent_by` always null + wrong event-publish argument order, same bug in `sales/invoices.ts`); frontend's own separate build errors logged as new item above, not yet fixed
- [P2] `/calls/:callId/stream` live-call-streaming WebSocket route has never actually been reachable — the `@fastify/websocket` plugin it depends on was never installed/registered; confirmed while fixing the build 2026-07-21, not fixed (real feature gap, not a build-config issue)
- [P1 — was Critical] Voice Bot serving a real client's (HBL Microfinance Bank) confidential script to every tenant across every sector when a tenant's own script was unset — DONE 2026-07-20, fixed at the code level (hardcoded fallback removed) plus Haier's own data corrected
- [P1 — was Critical] Voice Bot answering calls for tenants with zero allocated minutes instead of routing to a human — DONE 2026-07-20, reversed the "no quota row = unlimited" behavior platform-wide for all tenants/sectors
