...
Last updated: 2026-07-22 (2) — 5 high/medium-risk bugs (cross-department ticket access, module re-enable, queue creation, contact deletion, company-deletion crash) fixed and live-verified; Operations Manuals updated — see 2.16 below. Previous: 2026-07-22 — Full 188-row Test-Plan execution across 17 modules (145 pass, 15 real issues found and logged, 2 fixed live) — see 2.15 below. Previous: 2026-07-21 (2) — Custom Fields improvements, Governance consolidation, Push-to-Production built, 8-phase role-by-role Dashboard Audit (every role checked, several real bugs found and fixed) — see 2.14 below. Previous: 2026-07-21 (monorepo TypeScript build fixed across every backend package/module, no schema change — see 2.13 below). Previous: 2026-07-20 (Push 2 — critical cross-tenant Voice Bot identity leak + minutes enforcement fixed, see 2.12 below). Previous: 2026-07-20 (sector fields backfilled tenant-wide, full dept/queue/agent test coverage built and verified, API crash-resilience fix — see 2.11 below). Previous: 2026-07-18 (voice-bot ticket-type bug + cross-department SLA escalation leak, both fixed — see 2.10 below). Previous: 2026-07-17 push 2 (shared SMS gateway added; entitlement-drift bug fixed for real — see 2.9 below; voice bot cost tracking added; Nadia hold-message during ticket creation). Previous: 2026-07-17 push 1 (full role-by-role toggle/save audit across all 5 roles — see 2.8 below; also see CHANGE_LOG.md for the same date). Previous: 2026-07-12 (live phone-test fixes: quick-capture exemption from sector-required contact fields; voice-capture stop reliability; card-scan name/mobile accuracy; My Tasks quick-action buttons). Previous: 2026-07-11 (Field Mobile App: recovered from stash, re-tested against Supabase cloud backend; field-visit flow — assigned tasks, GPS check-in/complete, remarks, customer email; AI card scan + voice capture EN/UR/PA; offline queue; deal photo attachments). Previous: 2026-07-10 push 3 of 3 (agent dashboard 403 fixed at root cause; see AmanahCX-Roles-and-Flow.html v6.0). Push 2 of 3: reference document restructure + live Dashboard/Reports audit. Push 1 of 3: critical data-isolation fix, auto-contact-creation on ticket create, manager queue visibility, deployment pipeline sync.

## 2.16 5 Bugs Fixed and Verified from the Test-Plan Run — Operations Manuals Updated (2026-07-22, Push 2)
All 5 issues raised as significant findings in 2.15 below were fixed and live-verified this push:

1. **Cross-department ticket access (HIGH PRIORITY)** — any agent could view/edit any other agent's
   ticket in a different department by direct ticket ID. Added `assertTicketVisible()`, applied to all
   21 single-ticket routes in `tickets.ts`. Caught and fixed a parameter-count bug in the fix itself
   before confirming clean.
2. **Module re-enable bug (HIGH PRIORITY)** — toggling Voice Bot off then back on never restored it.
   Three stacked causes fixed together: module registry id (`voice`) didn't match the licensing key
   (`voice_bot`); `GET /api/v1/modules` never read the tenant admin's own enable/disable toggle at all;
   the toggle's write path never invalidated the 5-minute tenant cache. Also fixed a related bug: the
   licensing-enforcement safety net used the same wrong keys, silently zeroing voice permissions for
   every invited/updated user — fixed going forward; existing affected users still need a data
   correction (scope TBD, logged separately).
3. **Nobody could create a ticket queue** — added a dedicated `manageQueues` permission (manager+),
   matching the existing SLA-policies exemption pattern.
4. **Nobody could delete a contact** — benchmarked against Zendesk/Salesforce/HubSpot/Freshdesk first
   (all restrict this to admin-tier by default); kept the original admin-only intent and carved a narrow
   tenant_admin exception for this one action, rather than widening to manager-level.
5. **Company deletion crashed with linked contacts** — now auto-unlinks contacts/deals/activities/
   tickets before deleting (matches Zendesk/HubSpot's default; `sales_opportunities` already had
   `ON DELETE SET NULL` at the schema level).

Also removed 28 stale compiled `.js`/`.js.map`/`.d.ts` files checked into `modules/*/src/` across all 11
backend module packages (one was silently shadowing a live fix during verification) — added a
`.gitignore` rule to prevent recurrence.

Agreed direction with the user for the underlying pattern behind items 3 and 4: a granular, assignable
permissions system (matching Salesforce/Zendesk Enterprise/HubSpot/Freshdesk) rather than more one-off
role-rank exceptions. Logged to `BACKLOG.md` and the Bug Report as a planned feature, deliberately not
built this session — it needs its own dedicated design and testing pass.

All 4 Operations Manuals updated: Tenant Admin and Manager got real content additions reflecting items
2–4 above; Agent and Viewer reviewed for drift, confirmed no changes needed.
