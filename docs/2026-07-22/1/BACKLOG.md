...
### 0h. Cross-department ticket access via direct ID — NEEDS FIX
- **Verdict:** Real, confirmed 2026-07-22. An agent can view/edit any other agent's ticket (any department)
  by direct ID — the ticket list correctly scopes, but the single-ticket GET/PATCH routes only check
  tenant, not ownership. Same-tenant, not cross-tenant (tenant isolation itself re-verified solid).
- **Fix:** Add the same visibility-scoping check used in the list endpoint to the single-ticket
  GET/PATCH routes.

### 0g. Module re-enable doesn't restore access — NEEDS FIX
- **Verdict:** Real, confirmed 2026-07-22, root-caused. Turning a licensed module off then back on doesn't
  restore it to navigation — the module registry key (`voice`) doesn't match the licensing/entitlement
  system's key (`voice_bot`) for Voice Bot specifically. Confirmed via direct DB check + full server
  restart (not a caching issue).
- **Fix:** Reconcile the naming across the module registry, `MODULE_CATALOG`, and entitlement checks —
  needs a deliberate pass since it touches billing-adjacent code in several files at once.
