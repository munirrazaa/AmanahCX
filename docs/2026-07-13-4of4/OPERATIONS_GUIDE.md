...
- **Voice** — choose between the available Urdu voices (a warm female "helpdesk" voice or a polished male "support" voice)
- **Tone** — empathetic / professional / friendly / formal
- **Speaking speed** — a slider from slower to faster
- Plus the greeting text, behaviour instructions, and the usual ticket rules (queue, priority, self-service topics)

### Current status
- Talking to the bot and creating real tickets both work and have been tested.
- Not yet connected to an actual phone number — waiting on a SIP trunk (the phone-line connection) from Telecard, expected within days of this being written.
- Voice quality and response speed are still being tuned.
...
- Runs on a shared cloud service — a dedicated server improves both voice quality and response speed and is planned before go-live.
