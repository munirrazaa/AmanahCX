...
- **Verdict:** Do Soon
- **Status:** Open — discovered 2026-07-10 while fixing 0a, not fixed this session (scope too large to safely resolve alongside the immediate bug)
- **Why:** `tenant.settings.features.<name>` (boolean feature flags), `tenant.active_modules` (module-licensing array), and `user.permissions.<module>` (per-user level) all separately gate access to the same features, with no single source of truth. Confirmed concretely: Haier's `active_modules` correctly excludes `analytics`, but `settings.features.analytics` still returned `true`, allowing the paid `/api/v1/analytics/dashboard` summary endpoint to work despite the tenant never licensing it.
- **Detail:** Needs a proper audit of every route using `requireFeature()` vs `requireScope()` vs direct permission checks, and a decision on which single system should be authoritative, before consolidating the other two away.
...
### 14f. Nadia voice bot — admin portal roadmap (phase 1 done 2026-07-13)
- **Verdict:** Building incrementally, easy items first
- **Done 2026-07-13:** SIP trunk config fields, database-backed voice catalog (super admin managed), tenant-defined custom "no-ticket" reasons. See CHANGE_LOG Push 7.
- **Next, in order:**
  - Minutes-consumed counter + time-period reports (tenant admin + super admin visibility) — Medium effort, call duration already recorded per call
  - Super admin tops up minutes per tenant — Medium effort, new table + simple screen
