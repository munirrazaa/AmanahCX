...
4. The **Line Manager** dropdown automatically shows only managers from that same department — Sales and Complaints managers are hidden
5. Select the appropriate line manager
6. Choose the role (Agent, Manager, etc.) and click **Send Invite**

> Selecting a different department automatically clears the line manager selection, so you can never accidentally assign a Sales manager to a Support agent.

### Rules & limits
- The Line Manager dropdown only shows users with the Manager role in the same department
