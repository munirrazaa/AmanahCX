...
- [P2] Monorepo TypeScript build (`npm run build` / `tsc`) fails across `packages/api`, `packages/core`, `packages/shared` due to `rootDir` config mismatches, plus a couple of real type errors in `core/tenant.service.ts` and `core/email.service.ts` — doesn't affect production (which runs via `tsx` at runtime, not the compiled build), but should be fixed so the build check is trustworthy — IN PROGRESS 2026-07-20
