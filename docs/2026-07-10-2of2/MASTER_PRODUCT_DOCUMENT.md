...
Last updated: 2026-07-10 push 2 of 2 (reference document restructure + live Dashboard/Reports audit; see AmanahCX-Roles-and-Flow.html v5.9). Push 1 of 2: critical data-isolation fix, auto-contact-creation on ticket create, manager queue visibility, deployment pipeline sync.

## 2.6 Known Open Issue — Agent Permission Drift (found 2026-07-10)
A live audit (see `AmanahCX-Roles-and-Flow.html` → Dashboard Audit) found at least one agent whose stored `analytics` permission is `"none"` despite the code's own default for the agent role being `true`. This blocks that agent's Ops Dashboard with a 403, while the equivalent Reports endpoints remain accessible to them — an inconsistent access boundary. See `BACKLOG.md` item 0a for the fix plan.
