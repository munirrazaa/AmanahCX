...
## Known Issue — Some Agents May See "Access Denied" on Their Dashboard

**Status:** Under investigation, found 2026-07-10

A small number of agent accounts may see an error when opening their Dashboard, even though everything else in the system works normally for them. This happens when that specific account's permissions weren't set up correctly behind the scenes — it isn't something the agent did wrong.

**If this happens to one of your team members:** let support know which agent is affected — their permissions can be corrected directly without needing to recreate their account or affecting any of their existing tickets or work.
